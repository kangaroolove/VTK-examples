...
void VTKOpenGLWidget::createTestData() {
    ...
    std::cout << "matrixA=" << std::endl
              << *transformA->GetMatrix() << std::endl;

    vtkNew<vtkTransform> transformB;
    transformB->RotateZ(90);
    std::cout << "matrixB=" << std::endl
              << *transformB->GetMatrix() << std::endl;

    vtkNew<vtkMatrix4x4> matrixC;
    vtkNew<vtkMatrix4x4> matrixD;

    vtkMatrix4x4::Multiply4x4(transformA->GetMatrix(), transformB->GetMatrix(),
                              matrixC);
    vtkMatrix4x4::Multiply4x4(transformB->GetMatrix(), transformA->GetMatrix(),
                              matrixD);
    ...
    std::cout << "transformF=" << std::endl
              << *transformF->GetMatrix() << std::endl;

    qDebug() << "Concatenate";
    vtkNew<vtkTransform> transformG;
    transformG->Concatenate(transformA);
    transformG->Concatenate(transformB);
    std::cout << "transformG=" << std::endl
              << *transformG->GetMatrix() << std::endl;
}
