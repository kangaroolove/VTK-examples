...
public:
    ...
    void OnMouseMove() {
        ...
        // this->GetInteractor()->GetPicker()->Pick(eventPosition[0],
        // eventPosition[1], 0, mRenderer); double picked[3];
        // this->GetInteractor()->GetPicker()->GetPickPosition(picked);
        // std::cout<<"Picker "<<picked[0]<<", "<<picked[1]<<",
        // "<<picked[2]<<endl;
...
void SetImageActor(vtkImageActor *actor) { mImageActor = actor; }
...
void VTKOpenGLWidget::createTestData() {
    std::string dir =
        "D:/Standard test-data/Set N - Problematic "
        "dicom/CHENKAI/20230324080608/501";
    int index = 0;
    double level = 1245;
    ...
    // convert image orientation because some images are not based on RAI
    // coordinate
    ...
    calculateWorldPositionFromImageIJK(image, directionMatrix, centerPointIJK,
                                       centerPoint);
...
void VTKOpenGLWidget::calculateWorldPositionFromImageIJK(
    vtkImageData *image, vtkMatrix4x4 *directionMatrix, int in[3],
    double out[3]) {
    if (!image) return;
    ...
    for (int i = 0; i < 3; ++i) out[i] = vector[i] + origin[i];
}

void VTKOpenGLWidget::calculateImageIJKFromWorldPosition(
    vtkImageData *image, vtkMatrix4x4 *directionMatrix, double in[3],
    int out[3]) {
    if (!image) return;
...
void VTKOpenGLWidget::printArray(const std::string &name, double array[3]) {
    ;
    std::cout << name << " " << array[0] << ", " << array[1] << ", " << array[2]
              << endl;
}
